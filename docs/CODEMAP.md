...
| [`ChronicleState.cs`](../src/Chronicle.Core/ChronicleState.cs) | Durable Chronicle, combat, Lode, Source, and recorded-Attunement state plus deterministic Heartbeat advancement | `ChronicleState`, `WorldAddress` |
| [`ChronicleSaveCodec.cs`](../src/Chronicle.Core/ChronicleSaveCodec.cs) | Strict save v7 serialization, envelope dispatch, current/successor save mapping, and the private per-version document mirrors | `ChronicleSaveCodec.Serialize`, `ChronicleSaveCodec.Deserialize` |
...
| [`ChronicleStateValidation.cs`](../src/Chronicle.Core/ChronicleStateValidation.cs) | Strict loaded-state validation for Chronicle, combat, Loadout, and Holding invariants | Internal to `ChronicleSaveCodec` |
...
| [`WorldSubject.cs`](../src/Chronicle.Core/WorldSubject.cs) | One semantic durable-subject abstraction for every generated Chronicle subject: identity, kind, archetype, condition, bounded marks, and one optional bounded progress pair; it contains no pack visual identifier | `WorldSubject`, `WorldSubjectKind`, `WorldSubjectMark`, `WorldSubjectProgress`, `WorldSubjects` |
...
| --- | --- | --- |
| [`ChronicleApp.cs`](../src/Chronicle.Godot/ChronicleApp.cs) | Lifecycle, map request/composition, keyboard and HUD command routing, fixed pulse delivery, save I/O, Starting Vector choice, retained Goal 6A journeys, and the eight-stage Goal 6B rendered journey | Damage, range, Target eligibility, Load, timing, Source, carrying, forecast order, death, or persistence semantics |
| [`ChronicleHud.cs`](../src/Chronicle.Godot/ChronicleHud.cs) | Map-first 1600×900 frame: crisp 2× map, contextual material/combat decision rail, current/next capacity, keyboard-focusable action overlays, forecast/message surfaces, and command/Target events | Chronicle rules or alternate combat/power state; it presents snapshots and emits `ChronicleCommand` values |
...
| Source | Proves |
| --- | --- |
| [`Chronicle.Core.Checks/Program.cs`](../checks/Chronicle.Core.Checks/Program.cs) | Goal 6A retained combat plus Goal 6B identity, carrying, work, interruption, Attunement, loss/rebuild, death/replacement, strict v7 replay, malformed-state rejection, literal v6-through-pre-envelope migration, bounded WorldSubjects, the test-only fourth Modifier through production resolution/save validation, Core-copy isolation, and frozen pre-6C save bytes |
| [`Chronicle.Visuals.Checks/Program.cs`](../checks/Chronicle.Visuals.Checks/Program.cs) | P-GEN v2 combat and Goal 6B power-state vocabulary, semantic/render parity, adjacency, variant, layering, crop, overlap, numeric-address proof, and the frozen pre-6C Goal 6B render-plan digest |
| [`verify.ps1`](../checks/verify.ps1) | P-GEN authoring verification, packaged .NET builds, Core/Visual checks, the Godot editor build, exact four-file packaging isolation, World Atlas Inspector semantic and native visual parity, both Goal 6A journeys with their HUD proofs and saves, and the Goal 6B journey with its eight HUD proofs. Predecessor coverage lives in the Core literal save-migration fixtures, not in Godot journeys. |
